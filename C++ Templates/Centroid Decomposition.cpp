...
/*
 * Using solution to Balanced Cycles as code for template
...
// This method actually computes and returns the actual centroid of the tree (there's guaranteed to be one)
inline int Get_Centroid (int node, int prev, int sum) {
    for (size_t i=0; i<adj[node].size(); i++) {
        pii &next = adj[node][i];
        if (next.f == prev) continue;
        if (!is_centroid[next.f] && subtree_size[next.f] > sum / 2) return Get_Centroid(next.f, node, sum);
    }
    return node;
}
