...
int num_rows,num_cols,num_people,moves[4][2] = {{0,1},{1,0},{0,-1},{-1,0}},res;
unordered_set<int> rows[MAXN];
set<pii> vis;
queue<pii> q;

inline bool valid(int x, int y){
    if(x >= 1 && x <= num_rows && y >= 1 && y <= num_cols) return true;
    return false;
}
...
int main(){
    ...
    read(num_rows,num_cols,num_people);
    ...
    while(!q.empty()){
        ...
        for(int i=0; i<4; i++){
            int nx = x+moves[i][0], ny = y+moves[i][1];
            if(valid(nx,ny) && !vis.count({nx,ny}) && change(nx,ny)){
                rows[nx].insert(ny);
                vis.insert({nx,ny});
                q.push({nx,ny});
                res++;
